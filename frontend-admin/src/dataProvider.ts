...
getList: (resource: string, { pagination = {}, sort = {}, filter = {} }) => {
    ...
    } else if (filter && filter.brandId) {
      const brandId = filter.brandId;
      delete query.brandId;
      url = `${apiUrl}/public/brands/${brandId}/${resource}?${new URLSearchParams(query).toString()}`;
    } else {
      url = resource === "carts"
    ...
    return httpClient.get(url).then(({ json }) => {
      ...
      const logoBaseUrl = "http://localhost:8080/api/public/brands/logo/";
      ...
      if (resource === "carts") {
        ...
        const data = json.content.map(brand => ({
          id: brand.brandId,
          ...brand,
          logo: brand.logo ? `${logoBaseUrl}${brand.logo}` : null
        }));
        return {
          data,
...
create: async (resource: string, params: CreateParams): Promise<CreateResult> => {
    try {
      ...
      if (resource === "products") {
        url = `${apiUrl}/admin/categories/${params.data.categoryId}/brands/${params.data.brandId}/${resource}`;
        delete params.data.categoryId;
        delete params.data.brandId;
      ...
      throw error; // Hoac xử ly loi tuy theo nhu cau của ban
    }
  },
...
update: async (resource: string, params: UpdateParams): Promise<UpdateResult> => {
  ...
  getOne: async (resource: string, params: GetOneParams): Promise<GetOneResult> => {
    console.log("getOne called for resources: ", resource, "with params:", params)
    let url: string;
    if (resource === "carts") {
